...
### Indentation
Use 2 spaces per indentation level throughout all source files (TypeScript, HTML, Java).
...
### File Encoding
Save all source files as UTF-8. End every file with a single newline. Remove trailing whitespace from all lines. Exception: Markdown files may have trailing whitespace and no line-length limit.
